...
public class Client extends Thread {
	...
	public void doViewAction(JSONObject jo) {
		...
		if(jo.getString("type").contentEquals("register") ) {
			this.onRegister.register(jo, this, observers, alreadyRegistered);
	...
	private void listen() {
		...
		try {
			...
			while (listening) {
				String viewAction;
				viewAction = this.dataInputStream.readUTF();
				if (viewAction != null) {
					JSONObject jo = new JSONObject(new JSONTokener(viewAction));
